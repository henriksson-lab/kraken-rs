    ...
    T: Into<std::ffi::OsString> + Clone,
{
    ...
    match cli.command {
        ...
        Commands::Inspect {
            ...
            // Load taxonomy
            let mut taxonomy =
                match crate::taxonomy::Taxonomy::from_file(&taxo_file, memory_mapping) {
                    Ok(t) => t,
                    Err(e) => {
            ...
            // Load hash table
            let hash = match crate::compact_hash::CompactHashTable::from_file(
                &hash_file,
                memory_mapping,
            ) {
                Ok(h) => h,
            ...
            if skip_counts {
                ...
                if let Ok(idx_opts) = crate::types::IndexOptions::read_from_file(&opts_file) {
                ...
                let mut taxon_counters = crate::readcounts::TaxonCounters::new();
                ...
                if use_mpa_style {
                    let _ = crate::reports::report_mpa_style(
                        "/dev/stdout",
                        report_zero_counts,
                        &taxonomy,
                        &taxon_counters,
                    ...
                    let _ = crate::reports::report_kraken_style(
                        "/dev/stdout",
                        report_zero_counts,
                        false,
                        &taxonomy,
        ...
        Commands::Download {
            ...
            if taxonomy {
                if let Err(e) = crate::download::download_taxonomy(&db, skip_maps, protein) {
                    eprintln!("Error downloading taxonomy: {e}");
                    return 1;
                }
            }
            if let Some(ref lib_type) = library {
                if let Err(e) = crate::download::download_library(&db, lib_type, protein) {
        ...
        Commands::Clean { db } => {
            if let Err(e) = crate::download::clean_db(&db) {
